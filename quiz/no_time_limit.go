...
/*
func countResult(row []string) int {
    result, err :=  strconv.Atoi(row[1])
    if err != nil {
        log.Fatal("error of type result")
    }
    sumString := strings.Split(row[0], "+")
    first, err := strconv.Atoi(sumString[0])
    if err != nil {
        log.Fatal("error of type firstString")
    }
    second, err := strconv.Atoi(sumString[1])
    if err != nil {
        log.Fatal("error of type secondString")
    }
